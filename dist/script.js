          ...
          case "INPUT":
            if (item.getAttribute('type') === 'checkbox') {
              ...
              elem.forEach(function (box, j) {
                return j === i ? box.checked === true : box.checked = false;
              });
            } else {
...
var forms = function forms(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])("input[name='user_phone']"); //задаем объект для реакции сервера на наши ПОСТ запросы в модальном окне
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (e) {
      e.preventDefault(); //создаем сообщение о статусе отправки данных

      var statusMessage = document.createElement('div');
      statusMessage.classList.add("status");
      item.appendChild(statusMessage); //создаем переменную для отправки данных с формы
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        windows = document.querySelectorAll("[data-modal]"); //делаем переиспользуемую функцию для закрытия модальных окон
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); //делаем универсальный обработчик событий для всех табов
